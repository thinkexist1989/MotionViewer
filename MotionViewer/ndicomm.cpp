...
//    isStarted(false)
{
    ...
    ndiThread = QSharedPointer<NdiCommThread>(new NdiCommThread());
    connect(ndiThread.get(), &NdiCommThread::error, this, &NdiComm::processError);
    connect(ndiThread.get(), &NdiCommThread::timeout, this, &NdiComm::processTimout);
    connect(ndiThread.get(), &NdiCommThread::dataReady, this, [=](QVector<QVector3D> data){
            this->markers = data;
                emit this->dataReady(markers);});

    connect(ndiThread.get(), &NdiCommThread::serialOpenSuccess, this, [=](const QString& s){
        ui->openCloseButton->setText(tr("Close"));
        ui->openCloseButton->setIcon(QIcon(":/icon/res/stop.ico"));
        qDebug() << s;
    });

    connect(ndiThread.get(), &NdiCommThread::serialCloseSuccess, this, [=](const QString& s){
...
{
    if(!ndiThread->isRunning()) // serial not open
    {
        ...
        ndiThread->connectNDI(serialName,
                                 baudRate,
                                 parity,
                                 stopBits,
                                 dataBits,
    ...
    { // serial is already open
        ndiThread->disconnectNDI();
    }
}

void NdiComm::processError(const QString &s)
{
    //process errors from NdiCommThread
    qDebug() << tr("Status: Not running, %1.").arg(s);

}

void NdiComm::processTimout(const QString &s)
{
    //process timeout from NdiCommThread
